...
server.post("/clusters/", function (req, res, next) {
  const error = validateCluster(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.clusterName); // Generate a slug for new clusters.
    next();
  }
});
...
function validateCluster(cluster) {
  if (!cluster.clusterName) return "Cluster name is required.";
  if (!cluster.templateId) return "Template is required.";
  if (!cluster.volumeSize) return "Volume size is required.";
  return "";
}